...
def transform_fact_orders():
    ...
    df_orders2 = dataframes['order_item']
    df_orders2 = df_orders2.merge(dataframes['orders'], on='order_id')
    df_orders2 = df_orders2.merge(dataframes['product'], on='product_id')

    
    fact_orders_columns = dimension_columns.get('fact_orders')
    fact_order_detail_columns = dimension_columns.get('fact_order_items')
